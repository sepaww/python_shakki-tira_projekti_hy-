...
class TunnelVision_bot():
        ...
        for piece in self.piece_list:  
            ...
            for way in move:
                ...
                if self.game_matrix[way[0]][way[1]].team not in [self.team, -1]:
                    value+=(self.value_list[self.game_matrix[way[0]][way[1]].id][1]*2)
                print(value)    
                if value>max:
                    max=value
    ...
    def value_init(self):
        self.value_list=[(0,0), (1,5), (2,4), (2,4), (2,4), (2,10), (-20,20)]
        
        self.y_values=[-1,0,1,6,6,1,0,-1]
        self.x_values=[-1,0,1,2,2,1,0,-1]
    
    def pawn_values(self, i,j):
        temp_add=0
        if self.team==0:
            if i+1<=7:
                if j+1<=7:
